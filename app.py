...
def runQueryFor(instanceOf, entity):
    ...
    if entity in "author":
        ...
        relations = session.query(Book_Authors_Association).filter(
            Book_Authors_Association.author == author[0].name).all()
        publishers = []
        books = []
